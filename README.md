...
An open-source Codex plugin that connects to your local WhatsApp Web session, enforces self-only or explicitly whitelisted sending, and can accept prefixed automation commands only from your own WhatsApp self-chat.
...
Sending defaults to your linked number only. To use WhatsApp as a private command inbox, see [Owner-command automation](docs/AUTOMATION.md).
...
Open the **Codex Whatsapp Local** marketplace in the app’s plugin directory, install **WhatsApp for Codex**, restart the app, and open a new task. Ask: **Connect my WhatsApp account**.
