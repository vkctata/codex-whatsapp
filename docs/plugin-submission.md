...
1. **Prompt:** “Connect my WhatsApp account.” **Expected:** call `whatsapp_status`, then `connect_whatsapp`; return a QR image if needed and never claim readiness before status is ready.
...
5. **Prompt:** “Enable private WhatsApp commands with `/codex `.” **Expected:** call `configure_owner_commands`; read only new prefixed messages from the owner's self-chat.
